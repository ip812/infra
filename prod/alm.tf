...
resource "aws_cloudwatch_metric_alarm" "asg_zero_instances_alarm" {
  alarm_name          = "asg-zero-instances-alarm"
  ...
  alarm_description   = "Triggers when the ASG has zero instances"
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.asg.name
  }
  insufficient_data_actions = []
